    ...
    if request.method in ('POST', 'PUT', 'DELETE'):  # we need to parse xml
        ...
        if request.method == 'POST':
            if style_name:
                ...
                style.save()
            layer = None
            print '------------------------------- outside if layer_name exists :'+request.method
            if layer_name:
                try:
                    print '------------------------------- inside if layer_name exists :'+layer_name
                    ...
                    try:
                        ...
                        pass
            if layer:
                print '------------------------------- inside if layer FOUND :'+layer_name
                style.layer_styles.add(layer)
                print style.layer_styles
                style.save()
                print '------------------------------- style saved :'
                affected_layers.append(layer)
        elif request.method == 'PUT':  # update style in GN
            print '------------------------------- in methode  : '+request.method
            print '------------------------------- style_name: '+style_name
            if style_name:
                print '------------------------------- passewd if style_name: '+style_name
                style, created = Style.objects.get_or_create(name=style_name)
                style.sld_body = sld_body
                style.sld_url = url
                ...
                style.save()
                print '------------------------------- Style saved on PUT '
                print '------------------------------- number of relation style : %s' % str(style.layer_styles.count())
