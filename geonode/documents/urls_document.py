...
from .views import DocumentUploadView, DocumentUpdateView, KnowledgehubDocumentUploadView, KnowledgehubDocumentUpdateView
from . import views
from .models import KnowledgehubDocument
...
urlpatterns = [  # 'geonode.documents.views',
    ...
    url(r'^(?P<docid>\d+)/replace$', login_required(KnowledgehubDocumentUpdateView.as_view(context={'basemodel':basemodel})),
    ...
    url(r'^upload/?$', login_required(
        KnowledgehubDocumentUploadView.as_view(context={'basemodel':basemodel})), name=prefix+'_upload', kwargs={'basemodel':basemodel}),
