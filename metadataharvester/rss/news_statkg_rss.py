...
class Harvester(BaseHarvester):
    ...
    def harvest_all(self, **kwargs):
        ...
        # while True:
        response = requests.get(self.feed.format())
        if response.status_code == 200:
            NewsFeed = feedparser.parse(self.feed.format())
            for entry in NewsFeed.entries:
                ...
                #     'date': datetime.datetime.strptime(entry.published, '%a, %d %b %Y %H:%M:%S +0000').isoformat(),
                    'abstract': BeautifulSoup(entry.summary, "lxml").text,
