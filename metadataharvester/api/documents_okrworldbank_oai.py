...
def harvest_all(params={}):
    ...
    while True:
        ...
        save_document(docparams, specialparams, basemodel=KnowledgehubDocument)
...
def create_all_thumbnail():
    ...
    '''
    docs = KnowledgehubDocument.objects.filter(datasource=datasource)
    for doc in docs:
...
def create_thumbnail(doc_url, doc):
    ...
    # save thumbnail
    html_response = delayed_requests({'args':[doc_url], 'kwargs':{'allow_redirects':True}}, module=sys.modules[__name__])
    soup = BeautifulSoup(html_response.content, 'html.parser')
    elm_thumbnail = soup.find("img", {"id": "campaign-icon"})
    img_response = delayed_requests({'args':[urlparse.urljoin(html_response.url, elm_thumbnail.attrs['src'])], 'kwargs':{'allow_redirects':True}}, module=sys.modules[__name__])
